...
async function findTeamId(rawName) {
  ...
  for (const q of attempts) {
    ...
    debugLog.push({ query: q, resultCount: (data.response||[]).length, errors: data.errors });
    if (match) return match.team.id;
  }
  const err = new Error(`No team found for "${rawName}"`);
  err.debugLog = debugLog;
  throw err;
}
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: err.message, debugLog: err.debugLog });
  }
};
