...
// the likely cause.
function isWomensTeam(name) {
  return /\bwomen\b/i.test(name) || /\bw$/i.test(name.trim());
}

function pickBestMatch(response, query) {
  if (!response || !response.length) return null;
  const q = stripAccents(query).toLowerCase().trim();
  const menOnly = response.filter(r => !isWomensTeam(r.team.name));
  const pool = menOnly.length ? menOnly : response;
