...
async function findTeamId(rawName) {
  ...
  // The single-word fallback is only tried as a last resort, and only ever
  // accepts an exact name match — a generic word like "Club" or "Racing"
  // can otherwise match thousands of unrelated teams worldwide.
  const attempts = [
    { q: cleaned, strict: false },
    { q: stripAccents(rawName), strict: false },
    { q: rawName, strict: false },
    { q: firstWord, strict: firstWord.toLowerCase() !== cleaned.toLowerCase() }
  ];
  const tried = new Set();
  let first = true;
