...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!hId || !aId) {
      ...
      hId = await findTeamId(home);
      await sleep(350);
      aId = await findTeamId(away);
      await sleep(350);
    }
    ...
    res.status(200).json({ avgGoals, meetings: meetings.length, probHome, probDraw, probAway, debug: { resolvedHomeId: hId, resolvedAwayId: aId } });
  } catch (err) {
