...
function pickBestMatch(response, query) {
  ...
  const firstTeamsOnly = response.filter(r => !isNonFirstTeam(r.team.name));
  const pool = firstTeamsOnly.length ? firstTeamsOnly : response;
...
async function findTeamId(rawName) {
  ...
  for (const q of attempts) {
    ...
    if (data.response && data.response.length) {
      ...
      return { id: match.team.id, name: match.team.name, country: match.team.country, allCandidates: data.response.map(r => r.team.name) };
    }
  }
