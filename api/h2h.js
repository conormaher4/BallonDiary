...
function cleanTeamName(s) {
  const edgeTokens = new Set(['fc','cf','afc','sc','ac','as','cd','sad','ca','rc','rcd','club']);
  const connectorWords = new Set(['de','del','of','the','la','le']);
  let n = stripAccents(s).trim();
  n = n.split(/\s+/).filter(w => !connectorWords.has(w.toLowerCase())).join(' ');
  let words = n.split(/\s+/).filter(Boolean);
  while (words.length > 1 && edgeTokens.has(words[0].toLowerCase().replace(/[^a-z0-9]/g, ''))) words = words.slice(1);
  while (words.length > 1 && edgeTokens.has(words[words.length - 1].toLowerCase().replace(/[^a-z0-9]/g, ''))) words = words.slice(0, -1);
  return words.join(' ');
}
...
function pickBestMatch(pool, query, strict) {
...
async function findTeamId(rawName) {
  ...
  // Individual words are tried longest-first (more likely to be the club's
  // distinctive identifier, e.g. "Marseille" over "Olympique"), and only
  // ever accepted on an exact match — a short generic word like "Club" or
  // "Racing" would otherwise match thousands of unrelated teams worldwide.
  const words = [...new Set(cleaned.split(' ').filter(Boolean))].sort((a, b) => b.length - a.length);
  const attempts = [
    { q: cleaned, strict: false },
    { q: stripAccents(rawName), strict: false },
    { q: rawName, strict: false },
    ...words.map(w => ({ q: w, strict: true }))
  ];
  const tried = new Set();
  let first = true;
  for (const { q, strict } of attempts) {
    ...
    const firstTeamsOnly = candidates.filter(r => !isNonFirstTeam(r.team.name));
    if (firstTeamsOnly.length) {
      const match = pickBestMatch(firstTeamsOnly, q, strict);
      if (match) return { id: match.team.id, name: match.team.name, country: match.team.country, allCandidates: candidates.map(r => r.team.name) };
    }
  }
